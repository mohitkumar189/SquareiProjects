...
public class ChangePasswordTeacher extends AppCompatActivity implements ApiResponse {
                ...
                if (cd.isConnectingToInternet()) {
                    ...
                    if (isValidLoginDetails()) {
                        try {
                            ...
                            hm.put("currentpassword", old_password.getText().toString());
                            hm.put("newpassword", new_password.getText().toString());
                            hm.put("confirmpassword", confirm_password.getText().toString());
                            hm.put("authkey", Constant.AUTHKEY);
                            hm.put("userid", AppUtils.getUserId(context));

                            String url = getResources().getString(R.string.base_url) + getResources().getString(R.string.auth_changepassword);
                            new CommonAsyncTaskHashmap(1, context, ChangePasswordTeacher.this).getquery(url, hm);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
//
    ...
    private boolean isValidLoginDetails() {
        ...
        if (!old_password.getText().toString().equalsIgnoreCase("")
            ...
            if (old_password.getText().toString().equalsIgnoreCase("")) {
                isValidLoginDetails = false;
                Toast.makeText(context, getResources().getString(R.string.enter_old_pass), Toast.LENGTH_SHORT);
